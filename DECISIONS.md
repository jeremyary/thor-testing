...
**Open question, not yet investigated:** whether the OS image's own `bootc switch` pull path is subject to the same `registries.d` requirement, or uses different defaults/mechanism entirely. Given `bootc`/RHEM successfully delivered every OS image update so far, either it doesn't enforce `policy.json`'s `sigstoreSigned` the same way, or it has its own working `registries.d`-equivalent config not yet located. Worth a follow-up investigation before assuming the OS image's trust chain has the same gap this fix closes for workload images.
