...
- **Perses dashboards — root-caused 2026-08-12 (D027):** `PersesDashboard`/`PersesDatasource` CRs created in `gitops/observability/` (D021), but no Perses server ever deploys. Root cause, not just a symptom: COO 1.5.1's `uiplugin` controller almost certainly only reconciles `UIPlugin`-owned resources (its `Perses`/`PersesDashboard`/etc. watches exist to re-enqueue an *owning* `UIPlugin`, standard `Owns()` pattern) — our hand-created, ownerless `edge-perses` CR (D021, `spec: {}`) has zero reconcile log lines across its entire 3-day lifetime, confirmed via a live forced-reconcile test. Its `Available: True` status is a false positive (no Deployment/Pod/endpoints anywhere), most likely stamped by a webhook/default rather than real reconciliation. No operator upgrade exists to try (catalog only offers the already-installed 1.5.1). Practical fix: hand-roll our own Perses `Deployment`/`Service` reusing the operator's own image reference — scoped as a real follow-up feature build (Perses's Kubernetes-CR storage-backend config/RBAC), not attempted in this session. See `DECISIONS.md` D027.
